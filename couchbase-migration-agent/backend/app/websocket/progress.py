...
AWS-DMS-console style. Clients subscribe by migration_id; the MigrationEngine
calls `broadcast()` on every progress tick / phase change.
"""
...
async def migration_progress_ws(websocket: WebSocket, migration_id: str) -> None:
    ...
    try:
        while True:
            # Client doesn't need to send anything; keep the socket alive and
            # drain any pings so disconnects are detected promptly.
