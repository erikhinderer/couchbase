...
Every significant migration event (validation results, decisions, failures,
rollback reasons, user Q&A) is embedded with Qwen and written to a Couchbase
collection. recall() runs a native ANN vector query against the FTS index
defined in couchbase-memory/vector_index.json (created by scripts/init_memory.py
at container startup -- see the memory-init service in docker-compose.yml).

If that vector index is ever unavailable (e.g. momentarily still building on
first boot), recall() falls back to a brute-force cosine-similarity scan over
the same embeddings via N1QL, purely as a resilience net -- it is not the
...
# How many recent memory documents to pull back for the client-side
# cosine-similarity fallback. Memory volume here is agent episodes (validation
# results, chat turns, migration outcomes) -- thousands, not millions -- so a
# bounded N1QL scan is plenty fast without needing a real ANN index.
...
class AgentMemoryStore:
    ...
    def __init__(self) -> None:
        ...
        # Once native vector search fails once (e.g. the FTS index isn't
        # finished building yet), remember that so recall() stops retrying the
        # FTS path every call and goes straight to the N1QL fallback.
    ...
    async def recall(self, query_text: str, limit: int = 5) -> list[dict[str, Any]]:
        """Recall the most relevant past memory events for `query_text` via native
        Couchbase Vector Search, falling back to an in-process cosine-similarity
        scan if that's ever unavailable."""
        try:
    ...
    def _recall_via_cosine_fallback(self, query_vector: list[float], limit: int) -> list[dict[str, Any]]:
        """Brute-force semantic recall for clusters without native vector search:
        pull back the most recent memory documents that have an embedding, score
        each against the query vector in Python, and return the top `limit`."""
        cluster = self._connect()
        query = (
